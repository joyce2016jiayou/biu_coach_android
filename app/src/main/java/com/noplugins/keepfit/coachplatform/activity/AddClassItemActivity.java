...
//                startActivity(intent);
    ...
    private void getToken() {
        subscription = Network.getInstance("获取七牛云的token", this)
                .get_qiniu_token(new HashMap<>(),
                        new ProgressSubscriber<>("获取七牛云的token", new SubscriberOnNextListener<Bean<QiNiuToken>>() {
                            @Override
    ...
    protected void onResume() {
        ...
        if (class_jianjie_tv.length() == 0) {
            edit_class_jieshao.setHint(getResources().getText(R.string.edit_hint11));
        } else {
            edit_class_jieshao.setText(class_jianjie_tv);
        }

        if (shihe_renqun_tv.length() == 0) {
            edit_shihe_renqun.setHint(getResources().getText(R.string.edit_hint12));
        } else {
