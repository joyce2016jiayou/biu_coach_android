...
import com.noplugins.keepfit.coachplatform.util.ui.jiugongge.adapter.CCRRecyclerViewAdapter;
import com.noplugins.keepfit.coachplatform.util.ui.jiugongge.adapter.CCRRecyclerViewHolder;
import com.noplugins.keepfit.coachplatform.util.ui.jiugongge.adapter.CCRViewHolderHelper;
import com.noplugins.keepfit.coachplatform.util.ui.jiugongge.listen.CCROnItemChildClickListener;
import com.noplugins.keepfit.coachplatform.util.ui.jiugongge.listen.CCROnRVItemClickListener;
...
public class CCRSortableNinePhotoLayout extends RecyclerView implements CCROnItemChildClickListener, CCROnRVItemClickListener {
    ...
    public CCRSortableNinePhotoLayout(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CCRSortableNinePhotoLayout(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    private void initDefaultAttrs() {
        ...
        mOtherWhiteSpacing = CCRPhotoPickerUtil.dp2px(0);
    }

    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CCRSortableNinePhotoLayout);
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_plusEnable) {
            mPlusEnable = typedArray.getBoolean(attr, mPlusEnable);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_sortable) {
            mSortable = typedArray.getBoolean(attr, mSortable);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_deleteDrawable) {
            mDeleteDrawableResId = typedArray.getResourceId(attr, mDeleteDrawableResId);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_deleteDrawableOverlapQuarter) {
            mDeleteDrawableOverlapQuarter = typedArray.getBoolean(attr, mDeleteDrawableOverlapQuarter);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_maxItemCount) {
            mMaxItemCount = typedArray.getInteger(attr, mMaxItemCount);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_itemSpanCount) {
            mItemSpanCount = typedArray.getInteger(attr, mItemSpanCount);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_plusDrawable) {
            mPlusDrawableResId = typedArray.getResourceId(attr, mPlusDrawableResId);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_itemCornerRadius) {
            mItemCornerRadius = typedArray.getDimensionPixelSize(attr, 0);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_itemWhiteSpacing) {
            mItemWhiteSpacing = typedArray.getDimensionPixelSize(attr, mItemWhiteSpacing);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_otherWhiteSpacing) {
            mOtherWhiteSpacing = typedArray.getDimensionPixelOffset(attr, mOtherWhiteSpacing);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_placeholderDrawable) {
            mPlaceholderDrawableResId = typedArray.getResourceId(attr, mPlaceholderDrawableResId);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_editable) {
            mEditable = typedArray.getBoolean(attr, mEditable);
        } else if (attr == R.styleable.CCRSortableNinePhotoLayout_bga_snpl_itemWidth) {
    ...
    private void afterInitDefaultAndCustomAttrs() {
        if (mItemWidth == 0) {
            mItemWidth = (CCRPhotoPickerUtil.getScreenWidth() - mOtherWhiteSpacing) / mItemSpanCount;
        } else {
        ...
        addItemDecoration(new CCRSpaceItemDecoration(mItemWhiteSpacing / 2));
    ...
    private class ItemTouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
            if (actionState != ACTION_STATE_IDLE) {
                ...
                ((CCRRecyclerViewHolder) viewHolder).getViewHolderHelper().getImageView(R.id.iv_item_nine_photo_photo).setColorFilter(getResources().getColor(R.color.bga_pp_photo_selected_mask));
            }
        ...
        public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            ...
            ((CCRRecyclerViewHolder) viewHolder).getViewHolderHelper().getImageView(R.id.iv_item_nine_photo_photo).setColorFilter(null);
