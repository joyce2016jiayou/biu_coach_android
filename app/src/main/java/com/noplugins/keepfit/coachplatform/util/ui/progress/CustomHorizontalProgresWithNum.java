package com.noplugins.keepfit.coachplatform.util.ui.progress;
...
public class CustomHorizontalProgresWithNum extends ProgressBar {
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        //绘制图片
        float pianyiliang = HorizontalProgresTextOffset + textWidth;
        Bitmap bitmap = readBitMap(Mcontext, R.drawable.icon_rocket);
        canvas.drawBitmap(bitmap, (progressX + pianyiliang),-(bitmap.getHeight())/2, mPaint);
        //绘制未做走完的进度
        if (progressX + HorizontalProgresTextOffset + textWidth + bitmap.getWidth() < getWidth() - getPaddingLeft() - getPaddingRight()) {//进度走完了,不再画未走完的
            ...
            RectF mRectF2 = new RectF(progressX + HorizontalProgresTextOffset + textWidth + bitmap.getWidth(), getPaddingTop() - HorizontalProgresUnReachHeight / 2, realWidth, HorizontalProgresUnReachHeight / 2);//圆角 int left, int top, int right, int bottom
