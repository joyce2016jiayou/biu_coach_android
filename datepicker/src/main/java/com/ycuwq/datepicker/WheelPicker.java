...
public class WheelPicker<T> extends View {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		} else if (mTextPosition == 0){
			mFirstItemDrawX = mDrawnRect.right-mTextMaxWidth-20;
		} else {
			mFirstItemDrawX = mDrawnRect.left+30;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!TextUtils.isEmpty(mIndicatorText)) {
			canvas.drawText(mIndicatorText, mFirstItemDrawX + mTextMaxWidth / 2+30, mCenterItemDrawnY, mIndicatorPaint);
		}
	}
...
//                    computeDistanceToEndPoint(mScroller.getFinalY() % mItemHeight));
    ...
    public void setTextPosition(int position){
    	this.mTextPosition = position;
	}
	public int setTextPosition(){
